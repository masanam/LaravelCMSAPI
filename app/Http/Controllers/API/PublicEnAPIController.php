...
class PublicEnAPIController extends AppBaseController
{
...
/**
    ...
    public function getMenu()
    {
        ...
        foreach ($menu_list as $row) {
            ...
            $row->picture = $row->picture_en ;
            foreach ($row->children as $item){
                $item->title = $item->title_en ;
                $item->picture = $item->picture_en ;
                foreach ($item->children as $menu){
                    $menu->title = $menu->title_en ;
                    $menu->picture = $menu->picture_en ;
                    foreach ($menu->children as $child){
                        $child->title = $child->title_en ;
                        $child->picture = $child->picture_en ;
        ...
        foreach ($types as $row) {
            foreach ($row->brand as $item){
                ...
                foreach ($item->product as $prod){
                    ...
                    $prod->content = $prod->content_en ;
                    $prod->summary = $stringp ;
                    $prod->title = $prod->title_en ;
                }    
    ...
    {
        $prolog_corporate = \App\Models\Section::where('slug','prolog-corporate')->get();
    ...
    {
        $prolog_brand = \App\Models\Section::where('slug','prolog-brand')->get();
    ...
    public function getOrganizationData()
    {
        ...
        foreach ($Organisation as $row) {
            ...
            if (strpos($row->fullname, '&') !== false) {
                $slug = preg_replace("/[&]/", "n", $row->fullname);
            } else{
                $slug = $row->fullname;
            }
            ...
            $row->citizen = $row->citizen_en ;
            $row->position = $row->position_en ;
            $row->age = $row->age_en ;
            $row->education = $row->education_en ;
            $row->legal = $row->legal_en ;
            $row->experience = $row->experience_en ;
            $row->concurrent = $row->concurrent_en ;
            $row->affiliate = $row->affiliate_en ;
            $row->description = $row->description_en ;
        }
    ...
    public function getAllDirector()
    {
        ...
        foreach ($director as $row) {
            $row->category->title = $row->category->title_en;
            $string = $row->description_en;
    ...
    public function getDocumentCatalog()
    {
        ...
        foreach ($catalog as $row) {
            ...
            $row->seotitle = Str::slug($slug, '-');
            $row->title = $row->title_en ;
            $row->preview = $row->preview_en ;
            $row->filename = $row->filename_en ;
            $row->description = $row->description_en ;
        }
